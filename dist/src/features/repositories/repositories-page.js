...
import { RiArrowDownLine, RiCloseLine, RiFolder3Line, RiMagicLine, RiNotification2Line, RiRefreshLine, RiSearchLine, RiStarLine, } from "@remixicon/react";
...
import { ListsPage } from "../lists/lists-page.js";
export function RepositoriesPage({ state, onStateChange, onSync, syncing, syncError, syncWarning, syncSuccess, goToSettings, loading = false, }) {
    ...
    async function runAi(repo) { setAiLoading(repo.full_name); feedback(); try {
        ...
        feedback("", `${repo.full_name} 已完成 AI 分析`);
    }
    catch (error) {
        feedback(error instanceof Error ? error.message : "AI 分析失败");
    }
    finally {
        setAiLoading(null);
    } }
    ...
    async function applyBatchCategory(categoryValue) { if (!selected.size)
        ...
        feedback("", categoryName ? `已设置分类：${categoryName}` : "已设为未分类");
    }
    catch (error) {
        feedback(error instanceof Error ? error.message : "分类更新失败");
    } }
    return (_jsxs("div", { className: "mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8", children: [_jsxs("header", { className: "mb-5 flex items-end justify-between gap-4", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-xl font-semibold tracking-tight", children: "Star" }), _jsx("p", { className: "mt-1 text-sm text-muted-foreground", children: state.lastSyncAt ? `上次同步 ${new Date(state.lastSyncAt).toLocaleString("zh-CN")} · ${state.repositories.length} 个仓库` : `${state.repositories.length} 个仓库` })] }), _jsxs(Button, { onClick: onSync, loading: syncing, children: [_jsx(RiRefreshLine, { className: "size-4" }), "\u540C\u6B65 Star"] })] }), _jsx(StatusBanner, { error: syncError || actionError, warning: !syncError && !actionError ? syncWarning : "", success: !syncError && !actionError && !syncWarning ? syncSuccess : "" }), _jsxs(Toolbar, { className: "mb-5", "aria-label": "Stars \u5DE5\u5177\u680F", children: [_jsx(ToolbarGroup, { className: "min-w-[240px] flex-1", children: _jsxs(InputGroup, { className: "min-w-[220px]", children: [_jsx(InputGroupInput, { type: "search", "data-search-shortcut": "true", "aria-label": "\u641C\u7D22\u4ED3\u5E93", value: query, onChange: (event) => setQuery(event.target.value), placeholder: "\u641C\u7D22\u4ED3\u5E93\u3001\u63CF\u8FF0\u3001\u6807\u7B7E\u3001\u5907\u6CE8\u2026" }), _jsx(InputGroupAddon, { children: _jsx(RiSearchLine, { className: "size-4", "aria-hidden": "true" }) })] }) }), _jsx(ToolbarSeparator, {}), _jsxs(ToolbarGroup, { children: [_jsxs(Select, { "aria-label": "\u6309\u5206\u7C7B\u7B5B\u9009", value: category, onChange: (event) => setCategory(event.target.value), className: "min-w-32", children: [_jsx("option", { value: "", children: "\u5168\u90E8\u5206\u7C7B" }), _jsx("option", { value: "__uncategorized", children: "\u672A\u5206\u7C7B" }), sortedCategories.map((item) => _jsx("option", { value: item.name, children: item.name }, item.id))] }), _jsxs(Menu, { children: [_jsxs(MenuTrigger, { render: _jsx(Button, { variant: "outline", size: "default" }), children: [_jsx(RiFolder3Line, { className: "size-4" }), listFilter ? state.githubLists.find((item) => item.id === listFilter)?.name || "GitHub 列表" : "全部仓库"] }), _jsxs(MenuPopup, { className: "min-w-52", children: [_jsx(MenuItem, { onClick: () => setListFilter(""), children: "\u5168\u90E8\u4ED3\u5E93" }), state.githubLists.length ? _jsxs(_Fragment, { children: [_jsx(MenuSeparator, {}), _jsx(MenuGroupLabel, { children: "GitHub \u5217\u8868" }), state.githubLists.map((list) => _jsxs(MenuItem, { onClick: () => setListFilter(list.id), children: [_jsx(RiFolder3Line, { className: "size-4" }), _jsx("span", { className: "min-w-0 flex-1 truncate", children: list.name }), _jsx("span", { className: "text-xs text-muted-foreground", children: list.items.length })] }, list.id))] }) : null, _jsx(MenuSeparator, {}), _jsx(MenuItem, { onClick: () => setListManagerOpen(true), children: "\u7BA1\u7406\u5217\u8868\u2026" })] })] }), _jsxs(Select, { "aria-label": "\u6309\u8BED\u8A00\u7B5B\u9009", value: language, onChange: (event) => setLanguage(event.target.value), className: "min-w-32", children: [_jsx("option", { value: "", children: "\u5168\u90E8\u8BED\u8A00" }), languages.map((item) => _jsx("option", { children: item }, item))] }), _jsxs(Select, { "aria-label": "\u6392\u5E8F\u65B9\u5F0F", value: sort, onChange: (event) => setSort(event.target.value), className: "min-w-32", children: [_jsx("option", { value: "starred", children: "\u661F\u6807\u65F6\u95F4" }), _jsx("option", { value: "active", children: "\u6D3B\u8DC3\u65F6\u95F4" }), _jsx("option", { value: "stars", children: "Star \u6570\u91CF" })] }), _jsx(Tooltip, { content: direction === "desc" ? "当前倒序，点击切换正序" : "当前正序，点击切换倒序", children: _jsx(Button, { variant: "outline", size: "icon", "aria-label": direction === "desc" ? "切换为正序" : "切换为倒序", onClick: () => setDirection((value) => value === "desc" ? "asc" : "desc"), children: _jsx(RiArrowDownLine, { className: cn("size-4 transition-transform", direction === "asc" && "rotate-180") }) }) })] })] }), (query || language || category || listFilter) ? _jsxs("div", { className: "-mt-2 mb-4 flex flex-wrap items-center gap-2 text-xs text-muted-foreground", children: [_jsx("span", { children: "\u5F53\u524D\u7B5B\u9009\uFF1A" }), query ? _jsxs(Button, { size: "sm", variant: "outline", onClick: () => setQuery(""), children: ["\u641C\u7D22\uFF1A", query, " \u00D7"] }) : null, category ? _jsxs(Button, { size: "sm", variant: "outline", onClick: () => setCategory(""), children: ["\u5206\u7C7B\uFF1A", category === "__uncategorized" ? "未分类" : category, " \u00D7"] }) : null, language ? _jsxs(Button, { size: "sm", variant: "outline", onClick: () => setLanguage(""), children: ["\u8BED\u8A00\uFF1A", language, " \u00D7"] }) : null, listFilter ? _jsxs(Button, { size: "sm", variant: "outline", onClick: () => setListFilter(""), children: ["\u5217\u8868\uFF1A", state.githubLists.find((item) => item.id === listFilter)?.name || "未知", " \u00D7"] }) : null, _jsx(Button, { size: "sm", variant: "ghost", onClick: () => { setQuery(""); setCategory(""); setLanguage(""); setListFilter(""); }, children: "\u6E05\u9664\u7B5B\u9009" })] }) : null, selected.size ? _jsx("div", { className: "pointer-events-none fixed inset-x-0 bottom-5 z-50 flex justify-center px-4", children: _jsxs("div", { className: "pointer-events-auto flex max-w-[calc(100vw-2rem)] items-center gap-1 overflow-x-auto rounded-[100px] bg-primary px-3 py-2 text-primary-foreground shadow-2xl [scrollbar-width:none] [&::-webkit-scrollbar]:hidden", children: [_jsxs("span", { className: "shrink-0 px-3 text-sm font-medium", children: ["\u5DF2\u9009 ", selected.size, " \u4E2A"] }), _jsx(Button, { size: "sm", variant: "ghost", className: "shrink-0 rounded-[100px] text-primary-foreground hover:bg-primary-foreground/12 hover:text-primary-foreground", onClick: () => setSelected(new Set(filtered.map((item) => item.full_name))), children: "\u5168\u9009" }), _jsxs(Button, { size: "sm", variant: "ghost", className: "shrink-0 rounded-[100px] text-primary-foreground hover:bg-primary-foreground/12 hover:text-primary-foreground", onClick: batchSubscribe, children: [_jsx(RiNotification2Line, { className: "size-4" }), "\u8BA2\u9605"] }), _jsxs(Button, { size: "sm", variant: "ghost", className: "shrink-0 rounded-[100px] text-primary-foreground hover:bg-primary-foreground/12 hover:text-primary-foreground", disabled: !aiEnabled, onClick: () => { if (aiBatchRunning)
                                togglePause();
                            else
                                void runAiBatch(aiBatchFailures.length ? aiBatchFailures : undefined); }, children: [_jsx(RiMagicLine, { className: "size-4" }), aiBatchRunning ? (aiBatchPaused ? "继续分析" : `AI 分析 ${aiBatchProgress.done}/${aiBatchProgress.total}`) : aiBatchFailures.length ? `重试 ${aiBatchFailures.length}` : "AI 分析"] }), _jsxs(Menu, { children: [_jsx(MenuTrigger, { render: _jsx(Button, { size: "sm", variant: "ghost", className: "shrink-0 rounded-[100px] text-primary-foreground hover:bg-primary-foreground/12 hover:text-primary-foreground" }), children: "\u5206\u7C7B" }), _jsxs(MenuPopup, { children: [_jsx(MenuItem, { onClick: () => void applyBatchCategory("__uncategorized"), children: "\u672A\u5206\u7C7B" }), sortedCategories.map((item) => _jsx(MenuItem, { onClick: () => void applyBatchCategory(item.name), children: item.name }, item.id))] })] }), _jsxs(Button, { size: "sm", className: "shrink-0 rounded-[100px] border border-primary-foreground/30 bg-transparent text-primary-foreground hover:bg-destructive hover:text-white", onClick: () => setBatchUnstarOpen(true), children: [_jsx(RiStarLine, { className: "size-4" }), "\u53D6\u6D88 Star"] }), _jsx(Button, { size: "icon-sm", variant: "ghost", className: "shrink-0 rounded-full text-primary-foreground hover:bg-primary-foreground/12 hover:text-primary-foreground", "aria-label": "\u9000\u51FA\u591A\u9009", onClick: () => { setSelected(new Set()); setAiBatchFailures([]); }, children: _jsx(RiCloseLine, { className: "size-4" }) })] }) }) : null, loading ? _jsx("div", { className: "grid gap-3 md:grid-cols-2 xl:grid-cols-3", children: Array.from({ length: 9 }, (_, index) => _jsx(RepositoryCardSkeleton, {}, index)) })
                : state.repositories.length === 0 ? _jsx(Empty, { className: "min-h-[48vh] bg-card/30", children: _jsxs(EmptyContent, { children: [_jsx(EmptyIcon, { children: _jsx(RiStarLine, { className: "size-5" }) }), _jsx(EmptyTitle, { children: "\u8FD8\u6CA1\u6709\u4ED3\u5E93" }), _jsx(EmptyDescription, { children: "\u5148\u5728\u8BBE\u7F6E\u91CC\u8FDE\u63A5 GitHub\uFF0C\u7136\u540E\u540C\u6B65\u73B0\u6709 Star\u3002" }), _jsx(Button, { className: "mt-4", variant: "outline", onClick: goToSettings, children: "\u6253\u5F00\u8BBE\u7F6E" })] }) })
                    : filtered.length === 0 ? _jsx(Empty, { children: _jsxs(EmptyContent, { children: [_jsx(EmptyTitle, { children: "\u6CA1\u6709\u7B26\u5408\u5F53\u524D\u7B5B\u9009\u6761\u4EF6\u7684\u4ED3\u5E93" }), _jsx(EmptyDescription, { children: "\u8C03\u6574\u641C\u7D22\u6216\u7B5B\u9009\u6761\u4EF6\u540E\u518D\u8BD5\u3002" }), _jsx(Button, { className: "mt-3", size: "sm", variant: "outline", onClick: () => { setQuery(""); setCategory(""); setLanguage(""); }, children: "\u6E05\u9664\u7B5B\u9009" })] }) })
                        : _jsxs(_Fragment, { children: [_jsxs("div", { className: "mb-3 flex items-center justify-between text-xs text-muted-foreground", children: [_jsxs("span", { children: [filtered.length, " \u4E2A\u4ED3\u5E93"] }), _jsx("span", { children: sort === "starred" ? (direction === "desc" ? "最近星标" : "最早星标") : sort === "active" ? (direction === "desc" ? "最近活跃" : "最早活跃") : (direction === "desc" ? "最多 Star" : "最少 Star") })] }), _jsx("div", { className: "grid gap-3 md:grid-cols-2 xl:grid-cols-3", children: filtered.map((repo) => { const meta = metaFor(repo); return _jsx(RepositoryCard, { repository: repo, meta: meta, density: state.settings.density, aiEnabled: aiEnabled, aiLoading: aiLoading === repo.full_name, selected: selected.has(repo.full_name), selectionMode: selected.size > 0, releaseSubscribed: state.releaseSubscriptions.includes(repo.full_name), mutating: mutating.has(repo.full_name), onSelectedChange: (value) => setSelected((current) => { const next = new Set(current); if (value)
                                            ...
                                            next.delete(repo.full_name); return next; }), onEdit: () => setEditing(repo), onDetails: () => setDetails(repo), onOrganize: () => void runAi(repo), onToggleRelease: () => toggleRelease(repo.full_name), onUnstar: () => setUnstarTarget(repo) }, repo.full_name); }) })] }), _jsx(Modal, { open: listManagerOpen, title: "\u7BA1\u7406 GitHub \u5217\u8868", description: "\u521B\u5EFA\u5217\u8868\u5E76\u7BA1\u7406\u4ED3\u5E93\u5F52\u5C5E\u3002", onClose: () => setListManagerOpen(false), className: "max-w-6xl", children: _jsx(ListsPage, { state: state, onStateChange: onStateChange, goToSettings: () => goToSettings("account"), embedded: true }) }), _jsx(RepositoryEditor, { repository: editing, meta: editing ? metaFor(editing) : emptyMeta(), categories: state.categories, open: Boolean(editing), onClose: () => setEditing(null), onManageCategories: () => goToSettings("categories"), onSave: (meta) => editing ? updateMeta(editing, meta) : false }), _jsx(RepositoryDetail, { open: Boolean(details), repository: details, token: state.settings.githubToken, credentialConnected: state.settings.credentialConnected, onClose: () => setDetails(null), previousDisabled: detailIndex <= 0, nextDisabled: detailIndex < 0 || detailIndex >= filtered.length - 1, onPrevious: () => { if (detailIndex > 0)
                    ...
                    setUnstarTarget(null); }, children: _jsxs(AlertDialogPopup, { children: [_jsxs(AlertDialogHeader, { children: [_jsx(AlertDialogTitle, { children: "\u53D6\u6D88 Star\uFF1F" }), _jsxs(AlertDialogDescription, { children: ["\u5C06\u4ECE GitHub \u53D6\u6D88 Star\uFF0C\u5E76\u4ECE\u5F53\u524D Star \u96C6\u5408\u79FB\u9664 ", unstarTarget?.full_name ?? "该仓库", "\u3002\u6B64\u64CD\u4F5C\u9700\u8981\u518D\u6B21\u786E\u8BA4\u3002"] })] }), _jsxs(AlertDialogFooter, { children: [_jsx(AlertDialogClose, { render: _jsx(Button, { variant: "ghost" }), children: "\u53D6\u6D88" }), _jsx(Button, { variant: "destructive", onClick: () => { if (unstarTarget)
                                        void unstar(unstarTarget); }, children: "\u53D6\u6D88 Star" })] })] }) }), _jsx(AlertDialog, { open: batchUnstarOpen, onOpenChange: setBatchUnstarOpen, children: _jsxs(AlertDialogPopup, { children: [_jsxs(AlertDialogHeader, { children: [_jsx(AlertDialogTitle, { children: "\u53D6\u6D88\u8FD9\u4E9B\u4ED3\u5E93\u7684 Star\uFF1F" }), _jsxs(AlertDialogDescription, { children: ["\u5C06\u5BF9 GitHub \u6267\u884C\u53D6\u6D88 Star\uFF0C\u5E76\u4ECE\u5F53\u524D Star \u96C6\u5408\u79FB\u9664 ", selected.size, " \u4E2A\u4ED3\u5E93\u3002\u5931\u8D25\u9879\u4F1A\u4FDD\u7559\u9009\u4E2D\uFF0C\u65B9\u4FBF\u91CD\u8BD5\u3002"] })] }), _jsxs(AlertDialogFooter, { children: [_jsx(AlertDialogClose, { render: _jsx(Button, { variant: "ghost" }), children: "\u53D6\u6D88" }), _jsx(Button, { variant: "destructive", onClick: () => void batchUnstar(), children: "\u53D6\u6D88 Star" })] })] }) })] }));
}
