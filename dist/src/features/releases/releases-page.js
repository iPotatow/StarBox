...
import { mergeSuccessfulReleaseFeed, releaseStateKey } from "../../lib/storage.js";
function stateKey(release) { return releaseStateKey(release.id); }
function releaseCardKey(release) { return `${release.repoFullName}#${stateKey(release)}`; }
...
export function ReleasesPage({ state, onStateChange, goToSettings, goToStars, initialLoading = false }) {
    ...
    async function markRead(release, read = true) { try {
        await runOptimisticMutation(state, { ...state, releaseStates: { ...state.releaseStates, [stateKey(release)]: { read, updatedAt: new Date().toISOString() } } }, onStateChange, { operation: read ? "release.read" : "release.unread", payload: { releaseId: releaseStateKey(release.id), repoFullName: release.repoFullName, read } });
    }
    catch (reason) {
