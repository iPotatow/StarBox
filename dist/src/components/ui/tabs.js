...
export function TabsList({ className, variant = "default", ...props }) {
    return _jsxs(TabsPrimitive.List, { "data-slot": "tabs-list", "data-variant": variant, className: cn("relative flex w-fit items-center gap-1 text-muted-foreground", variant === "default" ? "min-h-9 rounded-lg bg-muted p-1" : "min-h-10 gap-1 border-b border-border bg-transparent", className), ...props, children: [_jsx(_Fragment, { children: props.children }), _jsx(TabsPrimitive.Indicator, { "data-slot": "tabs-indicator", className: cn("absolute bottom-0 left-0 transition-[width,translate] duration-200 ease-in-out", variant === "underline" ? "z-10 h-0.5 w-(--active-tab-width) translate-x-(--active-tab-left) translate-y-px bg-primary" : "inset-y-1 -z-0 h-auto w-(--active-tab-width) translate-x-(--active-tab-left) rounded-md bg-background shadow-xs") })] });
}
