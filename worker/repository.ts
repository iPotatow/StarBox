...
export class DataRepository {
  ...
  async recordActivity(type: string, _payload: unknown) {
    ...
    if (type === "releases_synced") await this.saveSettings({ "sync.releases_last_at": this.clock() });
    if (type === "forks_synced") await this.saveSettings({ "sync.forks_last_at": this.clock() });
  ...
  async saveReleaseAiSummary(fullName: string, releaseId: number, tagName: string, summary: Record<string, unknown>, modelId: string) {
    const normalizedFullName = fullName.trim();
    if (!normalizedFullName) throw new MutationRequestError("Repository full_name 不能为空");
    if (!Number.isSafeInteger(releaseId) || releaseId <= 0) throw new MutationRequestError("Release ID 无效");
    if (!summary || typeof summary !== "object" || Array.isArray(summary)) throw new MutationRequestError("Release AI 总结无效");
    const now = this.clock();
    await this.stmt(
      "INSERT INTO repositories (repository_id, full_name, github_repo_id, name, html_url, is_starred, release_ai_release_id, release_ai_tag, release_ai_summary_json, release_ai_model_id, release_ai_generated_at, github_snapshot_json) VALUES (?1, ?2, NULL, ?3, ?4, 0, ?5, ?6, ?7, ?8, ?9, '{}') ON CONFLICT(full_name) DO UPDATE SET release_ai_release_id = excluded.release_ai_release_id, release_ai_tag = excluded.release_ai_tag, release_ai_summary_json = excluded.release_ai_summary_json, release_ai_model_id = excluded.release_ai_model_id, release_ai_generated_at = excluded.release_ai_generated_at",
      crypto.randomUUID(),
      normalizedFullName,
      normalizedFullName.split("/").pop() || normalizedFullName,
      `https://github.com/${normalizedFullName}`,
      releaseId,
      tagName,
    ...
    return { repo_full_name: normalizedFullName, release_id: releaseId, tag_name: tagName, summary_json: JSON.stringify(summary), model_id: modelId, generated_at: now };
  }
  ...
  async bootstrap() {
    const queries = [
      ...
      ["releaseAiSummaries", "SELECT full_name AS repo_full_name, release_ai_release_id AS release_id, release_ai_tag AS tag_name, release_ai_summary_json AS summary_json, release_ai_model_id AS model_id, release_ai_generated_at AS generated_at FROM repositories WHERE release_ai_release_id IS NOT NULL AND release_ai_summary_json IS NOT NULL"],
    ...
    return {
      ...
      syncSummary: { stars: settings["sync.stars_last_at"] ?? null, releases: settings["sync.releases_last_at"] ?? null, forks: settings["sync.forks_last_at"] ?? null },
      ...entries,
      revision: 0,
      lastSeq: 0,
    };
