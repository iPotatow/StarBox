...
export async function handleSyncMutation(request: Request, env: StarBoxEnv, _identity: Identity) { if (!env.DB) return error("Worker 未配置 D1 DB", 503); try { const record = await body(request); rejectClientTenant(record); const repository = new DataRepository(env.DB); const operation = typeof record.operation === "string" ? record.operation : "mutation"; const result = await repository.mutate(operation, record.payload && typeof record.payload === "object" ? record.payload as Record<string, unknown> : record); return json({ ...result, state: {} }); } catch (reason) { return error(reason instanceof Error ? reason.message : "同步 mutation 失败", 400); } }
