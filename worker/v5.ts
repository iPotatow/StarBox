...
export async function handleSyncMutation(request: Request, env: StarBoxEnv, _identity: Identity) { if (!env.DB) return error("Worker 未配置 D1 DB", 503); try { const record = await body(request); rejectClientTenant(record); const repository = new DataRepository(env.DB); const operation = typeof record.operation === "string" ? record.operation : "mutation"; const result = await repository.mutate(operation, record.payload && typeof record.payload === "object" ? record.payload as Record<string, unknown> : record); await repository.recordActivity(operation, record.payload ?? {}); return json({ ...result, state: {} }); } catch (reason) { return error(reason instanceof Error ? reason.message : "同步 mutation 失败", 400); } }
export async function handleActivity(request: Request, env: StarBoxEnv, _identity: Identity) { if (!env.DB) return error("Worker 未配置 D1 DB", 503); const repository = new DataRepository(env.DB); if (request.method === "GET") { const url = new URL(request.url); return json({ items: await repository.listActivity(Math.min(100, Math.max(1, Number(url.searchParams.get("limit")) || 50))) }); } try { const record = await body(request); rejectClientTenant(record); const id = await repository.recordActivity(typeof record.type === "string" ? record.type : "event", record.payload ?? {}); return json({ id }); } catch (reason) { return error(reason instanceof Error ? reason.message : "Activity 写入失败", 400); } }
