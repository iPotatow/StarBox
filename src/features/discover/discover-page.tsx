...
import { ExternalLinkIcon, SearchIcon, SettingsIcon } from "../../lib/animated-icons";
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    ...
    <form className="mb-4 grid gap-3 rounded-xl border border-border bg-card p-3 shadow-card" onSubmit={(event) => { event.preventDefault(); void load(); }}>
      ...
      <div className="flex flex-wrap items-end gap-3">
        <FieldRoot className="w-auto gap-1">
        ...
        <FieldRoot className="w-auto gap-1"><FieldLabel className="text-xs text-muted-foreground">{t("语言", "Language")}</FieldLabel><Input className="w-36 text-foreground" value={language} onChange={(event) => setLanguage(event.target.value)} placeholder="Any / rust" /></FieldRoot>
        <FieldRoot className="w-auto gap-1"><FieldLabel className="text-xs text-muted-foreground">Topic</FieldLabel><Input className="w-40 text-foreground" value={topic} onChange={(event) => setTopic(event.target.value)} placeholder="Any / react" /></FieldRoot>
        <FieldRoot className="w-auto gap-1"><FieldLabel className="text-xs text-muted-foreground">{t("时间范围", "Period")}</FieldLabel><Select aria-label={t("时间范围", "Period")} className="text-foreground" value={String(days)} onValueChange={(value) => setDays(Number(value))} items={[{ value: "7", label: t("7 天", "7 days") }, { value: "30", label: t("30 天", "30 days") }, { value: "90", label: t("90 天", "90 days") }, { value: "365", label: t("1 年", "1 year") }]} /></FieldRoot>
        <Button type="submit" loading={loading}><SearchIcon className="size-4" aria-hidden="true" />{queryDirty ? t("重新搜索", "Search again") : t("搜索 GitHub", "Search GitHub")}</Button>
      </div>
