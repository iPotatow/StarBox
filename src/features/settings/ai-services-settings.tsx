import { RiArrowDownSLine, RiCheckLine, RiEyeLine, RiEyeOffLine, RiKey2Line, RiRefreshLine } from "@remixicon/react";
...
export function AiServicesSettings() {
  ...
  return (
    <div className="grid gap-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium">{t("模型服务", "Model services")} <span className="ml-1 text-muted-foreground">{data.services.length}</span></p><p className="mt-1 text-xs text-muted-foreground">{t("一个服务可以添加多个模型。API Key 由 Worker 加密保存。", "Each service can contain multiple models. API keys are encrypted by the Worker.")}</p></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => void load()} loading={loading}><RiRefreshLine className="size-4" aria-hidden="true" />{t("刷新", "Refresh")}</Button><Button size="sm" onClick={openCreate}>+ {t("添加模型服务", "Add model service")}</Button></div></div>
              ...
              {service.models.length ? service.models.map((model) => {
                ...
                return (
                  <div key={model.id} className={`inline-flex items-center gap-1 rounded-lg border p-1 text-xs ${isDefault ? "border-primary/40 bg-primary/5" : "border-border/70 bg-secondary/30"}`}>
                    ...
                    {isDefault ? <Badge size="sm" variant="success"><RiCheckLine className="size-3" aria-hidden="true" />{t("默认", "Default")}</Badge> : null}
      ...
      >
        <div className="grid gap-4">
          ...
          <Field label="API Key" description={serviceModal === "edit" && editingService?.credentialConfigured ? t("已保存；留空保持现有凭据。", "Already saved; leave blank to keep the existing credential.") : undefined}><InputGroup><InputGroupInput type={showKey ? "text" : "password"} autoComplete="off" value={serviceDraft.apiKey} onChange={(event) => setServiceDraft((current) => ({ ...current, apiKey: event.target.value }))} /><InputGroupAddon align="inline-end"><Button type="button" variant="ghost" size="icon-sm" onClick={() => setShowKey((value) => !value)} aria-label={showKey ? t("隐藏 API Key", "Hide API Key") : t("显示 API Key", "Show API Key")}>{showKey ? <RiEyeOffLine className="size-4" aria-hidden="true" /> : <RiEyeLine className="size-4" aria-hidden="true" />}</Button></InputGroupAddon></InputGroup></Field>
          ...
          <Collapsible className="rounded-xl border border-border/70">
            <CollapsibleTrigger render={<Button type="button" variant="ghost" className="h-auto w-full justify-between rounded-xl px-4 py-3 text-sm font-medium" />}>
              {t("高级设置", "Advanced settings")}<RiArrowDownSLine className="size-4" aria-hidden="true" />
            </CollapsibleTrigger>
