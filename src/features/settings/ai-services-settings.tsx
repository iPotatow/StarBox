    ...
    <div className="grid gap-5">
        ...
        {data.services.map((service) => (
          <article key={service.id} className="rounded-xl border border-border/70 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2"><span className="text-xs text-muted-foreground">{service.enabled ? t("已启用", "Enabled") : t("已停用", "Disabled")}</span><Switch checked={service.enabled} disabled={busy === `service:${service.id}`} onCheckedChange={(checked) => void toggleService(service, checked)} aria-label={t("启用模型服务", "Enable model service")} /><Button variant="ghost" size="icon-sm" aria-label={t("添加模型", "Add model")} onClick={() => { setModelService(service); setModelId(""); setModelName(""); }}>+</Button><Button variant="ghost" size="sm" onClick={() => void test(service)} loading={busy === `test:${service.id}`}><RiKey2Line className="size-4" />{t("测试", "Test")}</Button><Button variant="ghost" size="sm" onClick={() => openEdit(service)}>{t("编辑", "Edit")}</Button><Button variant="ghost" size="sm" onClick={() => setDeleteTarget({ type: "service", service })}>{t("删除", "Delete")}</Button></div>
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              {service.models.length ? service.models.map((model) => {
                const isDefault = model.id === data.defaultModelId;
                return <div key={model.id} className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1.5 text-xs ${isDefault ? "border-primary/40 bg-primary/5" : "border-border/70 bg-secondary/30"}`}><button type="button" className="font-medium outline-none" disabled={!service.enabled || !model.enabled || isDefault} onClick={() => void setDefault(model.id)}>{model.displayName || model.remoteModelId}</button>{isDefault ? <Badge size="sm" variant="success"><RiCheckLine className="size-3" />{t("默认", "Default")}</Badge> : null}<button type="button" className="ml-1 text-muted-foreground hover:text-destructive-foreground" aria-label={t("删除模型", "Delete model")} onClick={() => setDeleteTarget({ type: "model", service, modelId: model.id, modelName: model.displayName || model.remoteModelId })}>×</button></div>;
      ...
      <Modal open={Boolean(serviceModal)} title={serviceModal === "edit" ? t("编辑模型服务", "Edit model service") : t("添加模型服务", "Add model service")} description={t("凭据会在 Worker 端加密保存，不会从安全读取接口回显。", "Credentials are encrypted by the Worker and are never returned by safe read APIs.")} onClose={() => setServiceModal(null)} className="sm:max-w-xl">
        <div className="grid gap-4">
          ...
          <details className="rounded-xl border border-border/70 px-4 py-3"><summary className="cursor-pointer text-sm font-medium">{t("高级设置", "Advanced settings")}</summary><div className="mt-4"><Field label={t("自定义请求头", "Custom headers")} error={headersResult.error}><Textarea className="min-h-28 font-mono text-xs" spellCheck={false} value={serviceDraft.headersText} onChange={(event) => setServiceDraft((current) => ({ ...current, headersText: event.target.value }))} /></Field></div></details>
          <div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setServiceModal(null)}>{t("取消", "Cancel")}</Button><Button loading={busy === "save-service"} disabled={!serviceDraft.name.trim() || !serviceDraft.baseUrl.trim() || (serviceModal === "create" && !serviceDraft.apiKey.trim()) || Boolean(headersResult.error)} onClick={() => void saveService()}>{serviceModal === "edit" ? t("保存", "Save") : t("添加服务", "Add service")}</Button></div>
        </div>
      </Modal>

      <Modal open={Boolean(modelService)} title={t("添加模型", "Add model")} description={modelService ? `${modelService.name} · ${hostname(modelService.baseUrl)}` : undefined} onClose={() => setModelService(null)}>
        <div className="grid gap-4"><Field label={t("模型 ID", "Model ID")}><Input autoFocus placeholder="gpt-5.4" value={modelId} onChange={(event) => setModelId(event.target.value)} /></Field><Field label={t("显示名称（可选）", "Display name (optional)")}><Input value={modelName} onChange={(event) => setModelName(event.target.value)} /></Field><div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setModelService(null)}>{t("取消", "Cancel")}</Button><Button loading={busy === "add-model"} disabled={!modelId.trim()} onClick={() => void addModel()}>{t("添加模型", "Add model")}</Button></div></div>
      </Modal>
