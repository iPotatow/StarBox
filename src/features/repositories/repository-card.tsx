  ...
  return (
    <Card
      ...
      className={cn(
        ...
        compact ? "p-3" : "p-4",
        list && "xl:grid xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center xl:gap-5",
      )}
    >
      <div className="min-w-0">
        <div className="flex items-start gap-3">
          <Checkbox checked={selected} onCheckedChange={onSelectedChange} aria-label={`选择 ${repository.full_name}`} className="mt-2" />
          <img src={repository.owner.avatar_url} alt="" className={cn("shrink-0 rounded-lg bg-secondary object-cover", compact ? "size-9" : "size-10")} />
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 items-center gap-2">
              <a href={repository.html_url} target="_blank" rel="noreferrer" className="truncate text-sm font-semibold tracking-tight hover:underline">{repository.full_name}</a>
          ...
          <div className="flex items-center gap-1">
            <Tooltip content={releaseSubscribed ? "取消 Release 订阅" : "订阅 Release"}><Button variant="ghost" size="icon-sm" onClick={onToggleRelease} aria-label={releaseSubscribed ? "取消 Release 订阅" : "订阅 Release"}>{releaseSubscribed ? <RiNotification2Line className="size-4" /> : <RiNotificationOffLine className="size-4" />}</Button></Tooltip>
            {aiEnabled ? <Tooltip content="AI 整理"><Button variant="ghost" size="icon-sm" onClick={onOrganize} loading={aiLoading} aria-label="AI 整理">{!aiLoading ? <RiMagicLine className="size-4" /> : null}</Button></Tooltip> : null}
        ...
        <p className={cn("text-sm leading-6 text-muted-foreground", compact ? "mt-2 line-clamp-2" : list ? "mt-2 line-clamp-2" : "mt-3 line-clamp-3")}>{meta.aiSummary || repository.description || "暂无仓库描述"}</p>
      ...
      <div className={cn("mt-4", list && "xl:mt-0 xl:min-w-[320px]")}>
        ...
        <div className={cn("mt-3 flex items-center gap-1", list ? "xl:justify-end" : "justify-end")}>
          <Tooltip content="取消 Star"><Button variant="ghost" size="sm" onClick={onUnstar} disabled={mutating} aria-label="取消 Star"><RiStarLine className="size-3.5" />取消 Star</Button></Tooltip>
          <Button variant="ghost" size="sm" onClick={onEdit}>编辑</Button>
          <Button variant="ghost" size="sm" onClick={onTogglePin}>{meta.pinned ? "取消置顶" : "置顶"}</Button>
          <a href={repository.html_url} target="_blank" rel="noreferrer" aria-label="打开 GitHub" className="grid size-8 place-items-center rounded-md text-muted-foreground hover:bg-accent hover:text-foreground"><RiExternalLinkLine className="size-3.5" /></a>
        </div>
      </div>
    </Card>
  );
