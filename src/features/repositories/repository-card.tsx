...
export function RepositoryCard({ repository, meta, density, aiEnabled, aiLoading, selected, selectionMode = false, releaseSubscribed, mutating, onSelectedChange, onEdit, onDetails, onOrganize, onToggleRelease, onUnstar }: {
  ...
  return <Card render={<article />} data-selected={selected ? "true" : undefined} className={cn("group relative flex h-full min-w-0 flex-col overflow-hidden rounded-2xl border border-border/80 bg-card shadow-card transition-[background-color,border-color,box-shadow] duration-200 hover:border-foreground/20 hover:shadow-card-hover focus-within:border-foreground/25", selected && "border-foreground/35 ring-1 ring-foreground/10")}>
    <div className="absolute right-4 top-4 z-10"><Checkbox className="size-5" checked={selected} onCheckedChange={onSelectedChange} aria-label={`选择 ${repository.full_name}`} /></div>
    ...
    <footer className={cn("mt-auto border-t border-border/70", compact ? "px-3 py-2.5" : "px-4 py-3")}>
      ...
      <div className={cn("mt-2 flex items-center justify-start gap-0.5 transition-opacity", selectionMode && "pointer-events-none opacity-30")} aria-label="仓库操作">
        ...
        <Tooltip content={aiEnabled ? "AI 分析" : "请先在设置中连接 AI 服务"}><span><Button variant="ghost" size="icon-sm" onClick={onOrganize} loading={aiLoading} disabled={!aiEnabled} aria-label="AI 分析" className={actionClass}><RiMagicLine className="size-4" /></Button></span></Tooltip>
        ...
        <Tooltip content="编辑"><Button variant="ghost" size="icon-sm" onClick={onEdit} aria-label="编辑" className={actionClass}><RiSettings4Line className="size-4" /></Button></Tooltip>
