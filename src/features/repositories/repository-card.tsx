...
export function RepositoryCard({ repository, meta, density, aiEnabled, aiLoading, selected, selectionMode = false, releaseSubscribed, mutating, onSelectedChange, onEdit, onDetails, onOrganize, onToggleRelease, onUnstar }: {
  ...
  return <Card render={<article />} data-selected={selected ? "true" : undefined} className={cn("group relative flex h-full min-w-0 flex-col overflow-hidden rounded-2xl border border-border/80 bg-card shadow-card transition-[background-color,border-color,box-shadow] duration-200 hover:border-foreground/20 hover:shadow-card-hover focus-within:border-foreground/25 [content-visibility:auto] [contain-intrinsic-size:auto_260px]", selected && "border-foreground/35 ring-1 ring-foreground/10")}>
    ...
    <header className={cn("flex min-w-0 items-center gap-2.5 border-b border-border/70 bg-secondary/40 pr-14", compact ? "px-3 py-2.5" : "px-4 py-3.5")}>
      <div className={avatarClass} aria-hidden="true"><span>{repository.owner.login.slice(0, 1).toUpperCase()}</span><img src={repository.owner.avatar_url} alt="" loading="eager" decoding="async" referrerPolicy="no-referrer" className="absolute inset-0 size-full object-cover" onError={(event) => { event.currentTarget.style.display = "none"; }} /></div>
      <div className="min-w-0 flex-1"><Button variant="link" size="none" onClick={onDetails} title={repository.full_name} className="flex w-full min-w-0 justify-start truncate rounded-sm text-left text-sm font-semibold tracking-tight hover:text-primary">{repository.full_name}</Button><div className="mt-1 flex min-w-0 flex-wrap items-center gap-1.5">{meta.category ? <span title={meta.category} className="max-w-full truncate text-xs font-medium text-muted-foreground">{meta.category}</span> : null}{repository.archived ? <Badge variant="outline" size="sm" className="gap-1 rounded-md px-1.5 text-xs"><RiArchiveLine className="size-3" />{t("已归档", "Archived")}</Badge> : null}{releaseSubscribed ? <Badge variant="info" size="sm" className="gap-1 rounded-md px-1.5 text-xs"><RiNotification2Line className="size-3" />Release</Badge> : null}</div></div>
    </header>
    <div className={cn("min-w-0 flex-1", compact ? "px-3 pb-3 pt-2.5" : "px-4 pb-4 pt-3.5")}><p title={meta.aiSummary || repository.description || undefined} className={cn("line-clamp-2 break-words text-sm leading-5 text-muted-foreground [overflow-wrap:anywhere]", !compact && "sm:line-clamp-3")}>{meta.aiSummary || repository.description || ""}</p>{meta.note ? <p className="mt-2 line-clamp-2 max-w-full break-words rounded-md border-l-2 border-foreground/20 bg-secondary/35 px-2.5 py-1.5 text-xs leading-5 text-foreground/80 [overflow-wrap:anywhere]">{meta.note}</p> : null}{tags.length ? <div className="mt-2.5 flex min-w-0 flex-wrap gap-1.5">{tags.map((tag) => <Badge key={tag} title={tag} className="max-w-full min-w-0 truncate rounded-md px-1.5 text-xs font-medium">{tag}</Badge>)}{hiddenTagCount ? <Badge variant="outline" className="rounded-md px-1.5 text-xs font-medium">+{hiddenTagCount}</Badge> : null}</div> : null}</div>
    <footer className={cn("mt-auto border-t border-border/70", compact ? "px-3 py-2.5" : "px-4 py-3")}>
      ...
      <div className={cn("mt-2 flex items-center justify-start gap-0.5 transition-opacity", selectionMode && "pointer-events-none opacity-30")} aria-label={t("仓库操作", "Repository actions")}>
        <Tooltip content={releaseSubscribed ? t("取消 Release 订阅", "Unsubscribe from Releases") : t("订阅 Release", "Subscribe to Releases")}><Button variant="ghost" size="icon-sm" onClick={onToggleRelease} aria-label={releaseSubscribed ? t("取消 Release 订阅", "Unsubscribe from Releases") : t("订阅 Release", "Subscribe to Releases")} aria-pressed={releaseSubscribed} className={actionClass}>{releaseSubscribed ? <RiNotification2Line className="size-4" /> : <RiNotificationOffLine className="size-4" />}</Button></Tooltip>
        <Tooltip content={aiEnabled ? t("AI 分析", "AI analysis") : t("请先在设置中连接 AI 服务", "Connect an AI service in Settings first")}><span><Button variant="ghost" size="icon-sm" onClick={onOrganize} loading={aiLoading} disabled={!aiEnabled} aria-label={t("AI 分析", "AI analysis")} className={actionClass}><RiMagicLine className="size-4" /></Button></span></Tooltip>
        <Tooltip content={t("查看详情", "View details")}><Button variant="ghost" size="icon-sm" onClick={onDetails} aria-label={t("查看详情", "View details")} className={actionClass}><RiInformationLine className="size-4" /></Button></Tooltip>
        <Tooltip content={t("编辑", "Edit")}><Button variant="ghost" size="icon-sm" onClick={onEdit} aria-label={t("编辑", "Edit")} className={actionClass}><RiSettings4Line className="size-4" /></Button></Tooltip>
        <Tooltip content={t("在 GitHub 打开", "Open on GitHub")}><Button render={<a href={repository.html_url} target="_blank" rel="noreferrer" />} variant="ghost" size="icon-sm" aria-label={t("在 GitHub 打开", "Open on GitHub")} className={actionClass}><RiExternalLinkLine className="size-4" /></Button></Tooltip>
        <Tooltip content={t("取消 Star", "Unstar")}><Button variant="ghost" size="icon-sm" onClick={onUnstar} loading={mutating} aria-label={t("取消 Star", "Unstar")} className="text-muted-foreground transition-colors hover:text-destructive-foreground"><RiStarLine className="size-4" /></Button></Tooltip>
      </div>
    </footer>
  </Card>;
}
