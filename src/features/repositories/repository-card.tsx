import { BellIcon, BellOffIcon, InfoIcon, PencilIcon, StarIcon } from "lucide-react";
...
export function RepositoryCard({ repository, meta, aiEnabled, aiLoading, selected, selectionMode = false, releaseSubscribed, mutating, onSelectedChange, onEdit, onDetails, onOrganize, onToggleRelease, onUnstar }: {
  ...
  if (releaseSubscribed) statusItems.push({ key: "release", label: "Release", variant: "info", icon: <BellIcon className="size-3" aria-hidden="true" /> });
  ...
  return <BorderBeam active={aiLoading} size="md" colorVariant="colorful" strength={0.85} brightness={1.5} saturation={1.3} duration={1.8} theme="auto" data-repository-full-name={repository.full_name} className="h-full min-w-0">
    ...
    <div className="min-w-0 flex-1 px-4 pb-4 pt-3.5">
      ...
      {topics.length ? <div className="mt-2.5 flex min-w-0 flex-wrap items-center gap-1.5" aria-label={t("GitHub Topics；详情中可查看全部", "GitHub Topics; view all in details")}>{topics.map((tag) => <Badge key={tag} title={tag} className="min-w-0 truncate rounded-md px-1.5 text-xs font-medium">{tag}</Badge>)}{hiddenTopicCount ? <Badge variant="outline" title={t("在详情中查看全部 Topics", "View all Topics in details")} className="min-w-0 truncate rounded-md px-1.5 text-xs font-medium">+{hiddenTopicCount} · {t("详情", "Details")}</Badge> : null}</div> : null}
    </div>
    <footer className="mt-auto border-t border-border/70 px-4 py-3">
      ...
      <div className={cn("mt-2 flex items-center justify-start gap-0.5", selectionMode && "opacity-60")} aria-label={t("仓库操作", "Repository actions")}>
        ...
        <Tooltip content={t("取消 Star", "Unstar")}><Button variant="ghost" size="icon-sm" onClick={onUnstar} loading={mutating} disabled={selectionMode} aria-label={t("取消 Star", "Unstar")} className="text-muted-foreground hover:text-destructive-foreground"><StarIcon className="size-4" aria-hidden="true" /></Button></Tooltip>
      </div>
    </footer>
    </Card>
  </BorderBeam>;
