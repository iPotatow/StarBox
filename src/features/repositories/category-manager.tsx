...
export function CategoryManager({ open, state, onStateChange, onClose }: { open: boolean; state: PersistedState; onStateChange: (state: PersistedState) => void; onClose: () => void }) {
  ...
  async function commit(categories: CategoryDefinition[], repositoryMeta = state.repositoryMeta, changed?: CategoryDefinition) { const optimistic = { ...state, categories: categories.map((item, index) => ({ ...item, order: index })), repositoryMeta }; const category = changed ?? optimistic.categories[0]; setError(""); try { await runOptimisticMutation(state, optimistic, onStateChange, { operation: "category.update", payload: { id: category?.id ?? crypto.randomUUID(), name: category?.name ?? "未分类", color: category?.color ?? "neutral", sortOrder: category?.order ?? 0, locked: Boolean(category?.locked), categories: optimistic.categories } }); } catch (reason) { setError(reason instanceof Error ? reason.message : "分类保存失败"); } }
  function add() {
    const value = name.trim(); if (!value || state.categories.some((item) => item.name === value)) return;
    const category = { id: `cat-${Date.now()}`, name: value, color: "neutral", order: state.categories.length, locked: false }; void commit([...([...state.categories].sort((a, b) => a.order - b.order)), category], state.repositoryMeta, category); setName("");
  }
  function update(category: CategoryDefinition, patch: Partial<CategoryDefinition>) {
    const nextName = patch.name?.trim() || category.name;
    const categories = state.categories.map((item) => item.id === category.id ? { ...item, ...patch, name: nextName } : item).sort((a, b) => a.order - b.order);
    ...
    void commit(categories, repositoryMeta, { ...category, ...patch, name: nextName });
  }
  function move(index: number, delta: number) {
    const list = [...state.categories].sort((a, b) => a.order - b.order); const target = index + delta; if (target < 0 || target >= list.length) return;
    [list[index], list[target]] = [list[target], list[index]]; void commit(list, state.repositoryMeta, list[target]);
  }
  function remove(category: CategoryDefinition) {
    ...
    void commit(state.categories.filter((item) => item.id !== category.id), repositoryMeta, category);
  }
