...
import { batchStarAction, commitCanonicalMutation, organizeRepository, unstarRepository } from "../../lib/api";
...
export function RepositoriesPage({
  state, onStateChange, onSync, syncing, syncError, syncSuccess, goToSettings, loading = false,
}: {
  ...
  async function unstar(repo: Repository) { if (!hasGithubCredential) return goToSettings(); setMutating((current) => new Set(current).add(repo.full_name)); feedback(); const previous = state; const optimistic = { ...state, repositories: state.repositories.filter((item) => item.full_name !== repo.full_name) }; onStateChange(optimistic); try { await unstarRepository(state.settings.githubToken.trim(), repo.full_name); onStateChange(await commitCanonicalMutation(optimistic, { id: crypto.randomUUID(), operation: "unstar", payload: { fullName: repo.full_name } })); setSelected((current) => { const next = new Set(current); next.delete(repo.full_name); return next; }); feedback("", `已取消 Star：${repo.full_name}`); } catch (error) { onStateChange(previous); feedback(error instanceof Error ? error.message : "取消 Star 失败"); } finally { setMutating((current) => { const next = new Set(current); next.delete(repo.full_name); return next; }); } }
