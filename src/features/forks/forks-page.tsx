...
function upstreamFilterLabel(value: Exclude<UpstreamFilter, "all">, language: "zh-CN" | "en") { if (language === "en") return value === "behind" ? "Behind upstream" : value === "ahead" ? "Ahead of upstream" : value === "synced" ? "Synced" : "Unknown"; return value === "behind" ? "落后上游" : value === "ahead" ? "领先上游" : value === "synced" ? "已同步" : "未知"; }
function workflowFilterLabel(value: Exclude<WorkflowFilter, "all">, language: "zh-CN" | "en") { if (language === "en") return value === "success" ? "Succeeded" : value === "failure" ? "Failed" : value === "running" ? "Running" : "No runs"; return value === "success" ? "运行成功" : value === "failure" ? "运行失败" : value === "running" ? "运行中" : "无运行记录"; }
...
export function ForksPage({ state, onStateChange, goToSettings, initialLoading = false, bootstrapPending = false }: { state: PersistedState; onStateChange: StateChange; goToSettings: (tab?: string) => void; initialLoading?: boolean; bootstrapPending?: boolean }) {
  ...
  async function runWorkflow() {
    ...
    try { const result = await dispatchForkWorkflow(token, review.fullName, review.workflowId, review.ref, review.inputs); setSuccess(""); notify(t("Workflow 已触发", "Workflow dispatched"), result.message, "success"); setWorkflowReview(null); }
    catch (reason) { notify(t("Workflow 触发失败", "Failed to dispatch workflow"), reason instanceof Error ? reason.message : review.fullName, "error"); }
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    ...
    <FilterBar>
      ...
      <FilterBarDesktop aria-label={t("Fork 工具栏", "Fork toolbar")}>
        <FilterBarSearch><InputGroup className="min-w-[220px]"><InputGroupInput type="search" data-search-shortcut="true" aria-label={t("搜索 Fork", "Search forks")} value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder={t("搜索 Fork / 上游 / Workflow", "Search fork / upstream / workflow")} /><InputGroupAddon><SearchIcon className="size-4" aria-hidden="true" /></InputGroupAddon></InputGroup></FilterBarSearch>
        <FilterBarSeparator />
        <FilterBarControls>
    ...
    {(query || upstreamFilter !== "all" || workflowFilter !== "all") ? <FilterBarChips><span>{t("当前筛选：", "Filters:")}</span>{query ? <Button size="sm" variant="outline" onClick={() => { setQuery(""); setPage(1); }}>{t("搜索：", "Search: ")}{query} ×</Button> : null}{upstreamFilter !== "all" ? <Button size="sm" variant="outline" onClick={() => { setUpstreamFilter("all"); setPage(1); }}>{t("上游：", "Upstream: ")}{upstreamFilterLabel(upstreamFilter, uiLanguage)} ×</Button> : null}{workflowFilter !== "all" ? <Button size="sm" variant="outline" onClick={() => { setWorkflowFilter("all"); setPage(1); }}>Actions: {workflowFilterLabel(workflowFilter, uiLanguage)} ×</Button> : null}<Button size="sm" variant="ghost" onClick={() => { setQuery(""); setUpstreamFilter("all"); setWorkflowFilter("all"); setPage(1); }}>{t("清除筛选", "Clear filters")}</Button></FilterBarChips> : null}
    <ResponsiveDialog
    ...
    <AlertDialog open={Boolean(workflowReview)} onOpenChange={(open) => { if (!open && !dispatching) setWorkflowReview(null); }}>
      <AlertDialogPopup>
        <AlertDialogHeader>
          <AlertDialogTitle>{t("确认运行 Workflow？", "Confirm workflow dispatch?")}</AlertDialogTitle>
          <AlertDialogDescription>{t("运行前请检查目标仓库、Workflow、Ref 与输入参数。提交后会立即在 GitHub Actions 中触发。", "Review the target repository, workflow, ref, and inputs before dispatching. Confirming immediately triggers GitHub Actions.")}</AlertDialogDescription>
        </AlertDialogHeader>
        {workflowReview ? <div className="grid gap-2 rounded-xl border border-border bg-secondary/30 p-3 text-sm">
          <div className="grid grid-cols-[88px_minmax(0,1fr)] gap-2"><span className="text-muted-foreground">{t("仓库", "Repository")}</span><span className="min-w-0 break-all font-medium">{workflowReview.fullName}</span></div>
          ...
          <div className="grid grid-cols-[88px_minmax(0,1fr)] gap-2"><span className="text-muted-foreground">{t("输入参数", "Inputs")}</span>{Object.keys(workflowReview.inputs).length ? <pre className="max-h-32 min-w-0 overflow-auto whitespace-pre-wrap break-all rounded-md bg-background/70 p-2 text-xs">{JSON.stringify(workflowReview.inputs, null, 2)}</pre> : <span className="text-muted-foreground">{t("无", "None")}</span>}</div>
        </div> : null}
        <AlertDialogFooter>
          <AlertDialogClose render={<Button variant="ghost" disabled={dispatching} />}>{t("返回修改", "Back")}</AlertDialogClose>
          <Button loading={dispatching} disabled={!workflowReview} onClick={() => void runWorkflow()}>{t("确认运行", "Confirm run")}</Button>
        </AlertDialogFooter>
      </AlertDialogPopup>
    </AlertDialog>
  </div>;
