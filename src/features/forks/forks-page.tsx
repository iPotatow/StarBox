...
import { FilterBar, FilterBarControls, FilterBarDesktop, FilterBarMobile, FilterBarMobileControls, FilterBarSearch, FilterBarSeparator } from "../../components/patterns/filter-bar";
...
export function ForksPage({ state, onStateChange: _onStateChange, goToSettings, initialLoading = false }: { state: PersistedState; onStateChange: StateChange; goToSettings: (tab?: string) => void; initialLoading?: boolean }) {
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <PageHeader><PageHeaderContent><PageHeaderTitle>Fork</PageHeaderTitle><PageHeaderDescription>{t(`${forks.length} 个 Fork · 查看与上游的差异和 Actions 状态。`, `${forks.length} forks · compare upstream differences and Actions status.`)}</PageHeaderDescription></PageHeaderContent><Button onClick={() => void loadForks()} loading={loading} disabled={!hasGithubCredential}><RiRefreshLine className="size-4" />{t("刷新 GitHub", "Refresh GitHub")}</Button></PageHeader>
    <StatusBanner error={error} success={!error ? success : ""} />
    <FilterBar>
      <FilterBarMobile>
        <InputGroup><InputGroupInput type="search" data-search-shortcut="true" aria-label={t("搜索 Fork", "Search forks")} value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder={t("搜索 Fork / 上游 / Workflow", "Search fork / upstream / workflow")} /><InputGroupAddon><RiSearchLine className="size-4" aria-hidden="true" /></InputGroupAddon></InputGroup>
        <FilterBarMobileControls>
          ...
          <Button variant="outline" size="icon" aria-label={direction === "desc" ? t("切换为正序", "Switch to ascending") : t("切换为逆序", "Switch to descending")} onClick={() => { setDirection((value) => value === "desc" ? "asc" : "desc"); setPage(1); }}><RiArrowDownLine className={cn("size-4 transition-transform", direction === "asc" && "rotate-180")} aria-hidden="true" /></Button>
        </FilterBarMobileControls>
      </FilterBarMobile>
      <FilterBarDesktop aria-label={t("Fork 工具栏", "Fork toolbar")}>
        <FilterBarSearch><InputGroup className="min-w-[220px]"><InputGroupInput type="search" data-search-shortcut="true" aria-label={t("搜索 Fork", "Search forks")} value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder={t("搜索 Fork / 上游 / Workflow", "Search fork / upstream / workflow")} /><InputGroupAddon><RiSearchLine className="size-4" aria-hidden="true" /></InputGroupAddon></InputGroup></FilterBarSearch>
        <FilterBarSeparator />
        <FilterBarControls>
