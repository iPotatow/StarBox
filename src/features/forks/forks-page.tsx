...
export function ForksPage({ state, onStateChange: _onStateChange, goToSettings, initialLoading = false }: { state: PersistedState; onStateChange: (next: PersistedState) => void; goToSettings: (tab?: string) => void; initialLoading?: boolean }) {
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    ...
    <Toolbar className="mb-5" aria-label="Fork 工具栏"><ToolbarGroup className="min-w-[240px] flex-1"><InputGroup className="min-w-[220px]"><InputGroupInput type="search" data-search-shortcut="true" aria-label="搜索 Fork" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="搜索 Fork / Upstream / Workflow" /><InputGroupAddon><RiSearchLine className="size-4" aria-hidden="true" /></InputGroupAddon></InputGroup></ToolbarGroup><ToolbarSeparator /><ToolbarGroup><Select aria-label="筛选 Upstream 状态" value={upstreamFilter} onChange={(event) => { setUpstreamFilter(event.target.value as UpstreamFilter); setPage(1); }} className="min-w-32"><option value="all">全部 Upstream</option><option value="behind">Behind</option><option value="ahead">Ahead</option><option value="synced">已同步</option><option value="unknown">未知</option></Select><Select aria-label="筛选 Actions 状态" value={workflowFilter} onChange={(event) => { setWorkflowFilter(event.target.value as WorkflowFilter); setPage(1); }} className="min-w-32"><option value="all">全部 Actions</option><option value="success">成功</option><option value="failure">失败</option><option value="running">运行中</option><option value="none">无运行记录</option></Select><Select aria-label="Fork 排序方式" value={sort} onChange={(event) => { setSort(event.target.value as ForkSort); setPage(1); }} className="min-w-32"><option value="updated">更新时间</option><option value="behind">Behind 数量</option><option value="ahead">Ahead 数量</option><option value="name">名称</option></Select><Tooltip content={direction === "desc" ? "当前逆序，点击切换正序" : "当前正序，点击切换逆序"}><Button variant="outline" size="icon" aria-label={direction === "desc" ? "切换为正序" : "切换为逆序"} onClick={() => { setDirection((value) => value === "desc" ? "asc" : "desc"); setPage(1); }}><RiArrowDownLine className={cn("size-4 transition-transform", direction === "asc" && "rotate-180")} /></Button></Tooltip></ToolbarGroup></Toolbar>
    ...
    <Modal open={Boolean(selected)} title={selected?.fullName || "Fork"} description={selected?.parentFullName ? `Upstream ${selected.parentFullName}` : "Fork 详情"} onClose={() => setSelected(null)}>{selected ? <div className="grid gap-4"><div className="grid gap-2 rounded-xl border border-border p-3 text-sm sm:grid-cols-2"><div><div className="text-xs text-muted-foreground">Ahead</div><div className="mt-1 font-medium">{selected.aheadBy ?? "未知"}</div></div><div><div className="text-xs text-muted-foreground">Behind</div><div className="mt-1 font-medium">{selected.behindBy ?? "未知"}</div></div></div>{selected.latestWorkflow ? <div className="rounded-xl bg-secondary/50 p-3 text-sm"><div className="flex items-center gap-2">{workflowState(selected) === "success" ? <RiCheckboxCircleLine className="size-4 text-success-foreground" /> : workflowState(selected) === "failure" ? <RiErrorWarningLine className="size-4 text-destructive-foreground" /> : <RiLoader4Line className="size-4 animate-spin text-warning-foreground" />}Latest Action · {selected.latestWorkflow.name}</div><div className="mt-1 text-xs text-muted-foreground">{selected.latestWorkflow.conclusion || selected.latestWorkflow.status} · {new Date(selected.latestWorkflow.createdAt).toLocaleString("zh-CN")}</div></div> : null}<div className="rounded-xl border border-border p-4"><div className="mb-3"><h3 className="text-sm font-semibold">运行 GitHub Workflow</h3><p className="mt-1 text-xs text-muted-foreground">使用 workflow_dispatch 在当前 Fork 上手动触发 Workflow。需要 GitHub Token 具备 Actions 权限，且目标 Workflow 支持手动触发。</p></div>{selected.workflows.length ? <div className="grid gap-3"><Field label="Workflow"><Select value={workflowId} onChange={(event) => setWorkflowId(event.target.value)}>{selected.workflows.map((workflow) => <option key={workflow.id} value={workflow.id}>{workflow.name} · {workflow.state}</option>)}</Select></Field><Field label="Ref / Branch"><Input value={workflowRef} onChange={(event) => setWorkflowRef(event.target.value)} placeholder={selected.defaultBranch} /></Field><details className="rounded-xl border border-border px-3 py-2"><summary className="cursor-pointer text-sm font-medium">Advanced · Inputs JSON</summary><div className="mt-3"><Field label="Inputs JSON" error={workflowInputError}><Textarea rows={3} className="font-mono text-xs" value={workflowInputs} onChange={(event) => { setWorkflowInputs(event.target.value); setWorkflowInputError(""); }} aria-invalid={Boolean(workflowInputError)} /><span className="text-xs text-muted-foreground">当前 GitHub API 未提供 workflow_dispatch input schema；无参数保持 {`{}`} {`}`}，需要参数时填写 JSON。</span></Field></div></details><Button loading={dispatching} disabled={!workflowId || !workflowRef.trim()} onClick={() => void runWorkflow()}>运行 Workflow</Button></div> : <p className="text-sm text-muted-foreground">此 Fork 没有可见的 GitHub Actions Workflow，或当前 Token 无权读取 Workflow。</p>}</div><div className="flex flex-wrap gap-2">{selected.behindBy && selected.behindBy > 0 ? <Button onClick={() => void syncUpstream(selected)} loading={syncing === selected.fullName}>同步 upstream</Button> : null}<Button render={<a href={selected.htmlUrl} target="_blank" rel="noreferrer" />} variant="outline"><RiExternalLinkLine className="size-4" />打开 Fork</Button>{selected.parentHtmlUrl ? <Button render={<a href={selected.parentHtmlUrl} target="_blank" rel="noreferrer" />} variant="outline">打开 Upstream</Button> : null}</div></div> : null}</Modal>
  </div>;
}
