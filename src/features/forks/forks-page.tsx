...
export function ForksPage({ state, onStateChange: _onStateChange, goToSettings, initialLoading = false }: { state: PersistedState; onStateChange: (next: PersistedState) => void; goToSettings: (tab?: string) => void; initialLoading?: boolean }) {
  ...
  const loadForks = useCallback(async () => {
    ...
    try { const next = await fetchForkRepositories(token); setForks(next); setSuccess(""); notify("Fork 已刷新", `${next.length} 个仓库 · Upstream / Actions 状态已更新`, "success"); }
  ...
  async function syncUpstream(fork: ForkRepository) {
    ...
    try { const result = await syncForkUpstream(token, fork.fullName, fork.defaultBranch); setSuccess(""); notify("Upstream 已同步", result.message || fork.fullName, "success"); const detail = await fetchForkDetails(token, fork.fullName); setForks((current) => current.map((item) => item.fullName === detail.fullName ? detail : item)); setSelected(detail); }
  ...
  async function runWorkflow() {
    ...
    catch (reason) { setWorkflowInputError(reason instanceof Error ? reason.message : "Inputs JSON 无效"); return; }
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <header className="mb-5 flex items-end justify-between gap-4"><div><h1 className="text-xl font-semibold tracking-tight">Fork</h1><p className="mt-1 text-sm text-muted-foreground">GitHub 中检测到 {forks.length} 个 Fork 仓库；检查 Upstream divergence 与 Actions，不在 StarBox 创建 Fork。</p></div><Button onClick={() => void loadForks()} loading={loading} disabled={!hasGithubCredential}><RiRefreshLine className="size-4" />刷新 GitHub</Button></header>
    <StatusBanner error={error} success={!error ? success : ""} />
    <Toolbar className="mb-5" aria-label="Fork 工具栏"><ToolbarGroup className="min-w-[240px] flex-1"><div className="relative w-full"><RiSearchLine className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="w-full min-w-[220px] pl-9" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="搜索 Fork / Upstream / Workflow" /></div></ToolbarGroup><ToolbarSeparator /><ToolbarGroup><Select value={upstreamFilter} onChange={(event) => { setUpstreamFilter(event.target.value as UpstreamFilter); setPage(1); }} className="min-w-32"><option value="all">全部 Upstream</option><option value="behind">Behind</option><option value="ahead">Ahead</option><option value="synced">已同步</option><option value="unknown">未知</option></Select><Select value={workflowFilter} onChange={(event) => { setWorkflowFilter(event.target.value as WorkflowFilter); setPage(1); }} className="min-w-32"><option value="all">全部 Actions</option><option value="success">成功</option><option value="failure">失败</option><option value="running">运行中</option><option value="none">无运行记录</option></Select><Select value={sort} onChange={(event) => { setSort(event.target.value as ForkSort); setPage(1); }} className="min-w-32"><option value="updated">更新时间</option><option value="behind">Behind 数量</option><option value="ahead">Ahead 数量</option><option value="name">名称</option></Select><Tooltip content={direction === "desc" ? "当前逆序，点击切换正序" : "当前正序，点击切换逆序"}><Button variant="outline" size="icon" onClick={() => { setDirection((value) => value === "desc" ? "asc" : "desc"); setPage(1); }}><RiArrowDownLine className={cn("size-4 transition-transform", direction === "asc" && "rotate-180")} /></Button></Tooltip></ToolbarGroup></Toolbar>
    ...
    {!hasGithubCredential ? <div className="rounded-xl border border-dashed border-border p-8 text-center"><p className="text-sm text-muted-foreground">连接 GitHub 凭据后才能读取已 Fork 仓库。</p><Button className="mt-3" variant="outline" onClick={() => goToSettings("account")}><RiSettings4Line className="size-4" />打开设置</Button></div>
      : pageLoading ? <ListSkeleton rows={8} />
        : <Card render={<section />} className="overflow-hidden rounded-xl shadow-card"><div className="grid grid-cols-[minmax(0,1fr)_110px_140px_140px] gap-3 border-b border-border bg-secondary/35 px-4 py-2 text-xs font-medium text-muted-foreground max-md:grid-cols-[minmax(0,1fr)_90px]"><span>Repository / Upstream</span><span className="max-md:hidden">Divergence</span><span className="max-md:hidden">Actions</span><span className="text-right">操作</span></div>{visible.map((fork) => { const status = upstreamState(fork); const actionStatus = workflowState(fork); return <article key={fork.id} className="grid grid-cols-[minmax(0,1fr)_110px_140px_140px] items-center gap-3 border-b border-border px-4 py-3 last:border-b-0 max-md:grid-cols-[minmax(0,1fr)_90px]"><div className="min-w-0"><Button variant="link" size="none" onClick={() => void inspectFork(fork)} className="max-w-full truncate text-left text-sm font-semibold">{fork.fullName}</Button><div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span className="truncate">Upstream: {fork.parentFullName || "未知"}</span></div><div className="mt-1 hidden flex-wrap gap-x-2 text-xs text-muted-foreground max-md:flex"><span>Ahead {fork.aheadBy ?? "?"}</span><span>Behind {fork.behindBy ?? "?"}</span><span>Actions {fork.latestWorkflow ? (fork.latestWorkflow.conclusion || fork.latestWorkflow.status) : "—"}</span></div></div><div className="text-xs max-md:hidden">{fork.aheadBy == null || fork.behindBy == null ? <Badge>unknown</Badge> : <div className="flex gap-1"><Badge>{fork.aheadBy} ahead</Badge><Badge>{fork.behindBy} behind</Badge></div>}</div><div className="min-w-0 text-xs max-md:hidden">{fork.latestWorkflow ? <a href={fork.latestWorkflow.htmlUrl} target="_blank" rel="noreferrer" className="flex min-w-0 items-center gap-1 hover:underline"><span className={cn("size-2 shrink-0 rounded-full", actionStatus === "success" ? "bg-emerald-500" : actionStatus === "failure" ? "bg-destructive" : "bg-warning")} /><span className="truncate">{fork.latestWorkflow.name} · {fork.latestWorkflow.conclusion || fork.latestWorkflow.status}</span></a> : "—"}</div><div className="flex justify-end gap-1">{status === "behind" ? <Tooltip content="同步 upstream"><Button size="sm" variant="outline" loading={syncing === fork.fullName} onClick={() => void syncUpstream(fork)}>同步</Button></Tooltip> : null}<Tooltip content="详情 / Workflow"><Button size="icon-sm" variant="ghost" loading={detailLoading === fork.fullName} onClick={() => void inspectFork(fork)}><RiRefreshLine className="size-4" /></Button></Tooltip><a href={fork.htmlUrl} target="_blank" rel="noreferrer"><Button size="icon-sm" variant="ghost" aria-label="打开 Fork"><RiExternalLinkLine className="size-4" /></Button></a></div></article>; })}{!visible.length ? <div className="grid min-h-56 place-items-center p-8 text-center text-sm text-muted-foreground"><div><RiGitForkLine className="mx-auto size-6" /><p className="mt-3">GitHub 中暂无符合条件的 Fork 仓库</p></div></div> : null}</Card>}
    ...
    <Modal open={Boolean(selected)} title={selected?.fullName || "Fork"} description={selected?.parentFullName ? `Upstream ${selected.parentFullName}` : "Fork 详情"} onClose={() => setSelected(null)}>{selected ? <div className="grid gap-4"><div className="grid gap-2 rounded-xl border border-border p-3 text-sm sm:grid-cols-2"><div><div className="text-xs text-muted-foreground">Ahead</div><div className="mt-1 font-medium">{selected.aheadBy ?? "未知"}</div></div><div><div className="text-xs text-muted-foreground">Behind</div><div className="mt-1 font-medium">{selected.behindBy ?? "未知"}</div></div></div>{selected.latestWorkflow ? <div className="rounded-xl bg-secondary/50 p-3 text-sm"><div className="flex items-center gap-2"><RiCheckboxCircleLine className="size-4" />Latest Action · {selected.latestWorkflow.name}</div><div className="mt-1 text-xs text-muted-foreground">{selected.latestWorkflow.conclusion || selected.latestWorkflow.status} · {new Date(selected.latestWorkflow.createdAt).toLocaleString("zh-CN")}</div></div> : null}<div className="rounded-xl border border-border p-4"><div className="mb-3"><h3 className="text-sm font-semibold">运行 GitHub Workflow</h3><p className="mt-1 text-xs text-muted-foreground">使用 workflow_dispatch 在当前 Fork 上手动触发 Workflow。需要 GitHub Token 具备 Actions 权限，且目标 Workflow 支持手动触发。</p></div>{selected.workflows.length ? <div className="grid gap-3"><Field label="Workflow"><Select value={workflowId} onChange={(event) => setWorkflowId(event.target.value)}>{selected.workflows.map((workflow) => <option key={workflow.id} value={workflow.id}>{workflow.name} · {workflow.state}</option>)}</Select></Field><Field label="Ref / Branch"><Input value={workflowRef} onChange={(event) => setWorkflowRef(event.target.value)} placeholder={selected.defaultBranch} /></Field><Field label="Inputs JSON"><Textarea rows={3} className="font-mono text-xs" value={workflowInputs} onChange={(event) => { setWorkflowInputs(event.target.value); setWorkflowInputError(""); }} aria-invalid={Boolean(workflowInputError)} />{workflowInputError ? <span className="text-xs text-destructive-foreground">{workflowInputError}</span> : <span className="text-xs text-muted-foreground">无 inputs 时保持 {`{}`} {`}`}；需要参数时填写例如 {`{"environment":"production"}`}。</span>}</Field><Button loading={dispatching} disabled={!workflowId || !workflowRef.trim()} onClick={() => void runWorkflow()}>运行 Workflow</Button></div> : <p className="text-sm text-muted-foreground">此 Fork 没有可见的 GitHub Actions Workflow，或当前 Token 无权读取 Workflow。</p>}</div><div className="flex flex-wrap gap-2">{selected.behindBy && selected.behindBy > 0 ? <Button onClick={() => void syncUpstream(selected)} loading={syncing === selected.fullName}>同步 upstream</Button> : null}<a href={selected.htmlUrl} target="_blank" rel="noreferrer"><Button variant="outline"><RiExternalLinkLine className="size-4" />打开 Fork</Button></a>{selected.parentHtmlUrl ? <a href={selected.parentHtmlUrl} target="_blank" rel="noreferrer"><Button variant="outline">打开 Upstream</Button></a> : null}</div></div> : null}</Modal>
  </div>;
}
