...
export function ForksPage({ state, onStateChange: _onStateChange, goToSettings, initialLoading = false }: { state: PersistedState; onStateChange: StateChange; goToSettings: (tab?: string) => void; initialLoading?: boolean }) {
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <PageHeader><PageHeaderContent><PageHeaderTitle>Fork</PageHeaderTitle><PageHeaderDescription>{t(`${forks.length} 个 Fork · 查看与上游的差异和 Actions 状态。`, `${forks.length} forks · compare upstream differences and Actions status.`)}</PageHeaderDescription></PageHeaderContent><Button onClick={() => void loadForks()} loading={loading} disabled={!hasGithubCredential}><RefreshCwIcon className="size-4" />{t("刷新 GitHub", "Refresh GitHub")}</Button></PageHeader>
