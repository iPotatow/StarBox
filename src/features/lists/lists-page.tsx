...
export function ListsPage({ state, onStateChange, goToSettings }: { state: PersistedState; onStateChange: (state: PersistedState) => void; goToSettings: () => void }) {
  ...
  async function createList() {
    ...
    try { const created = await createGithubList(token, name.trim(), description.trim(), createPrivate); const lists = [...state.githubLists, created]; await runOptimisticMutation(state, { ...state, githubLists: lists, lastListSyncAt: new Date().toISOString() }, onStateChange, { operation: "list.create", payload: { listId: created.id, name: created.name, description: created.description, isPrivate: created.isPrivate } }); setSelectedId(created.id); setCreateOpen(false); setName(""); setDescription(""); setCreatePrivate(false); setSuccess(`已创建 List：${created.name}`); }
  ...
  async function saveList(list: GithubStarList) {
    setSaving(true); setError(""); try { const next = await updateGithubList(token, list.id, list.name, list.description, list.isPrivate); await runOptimisticMutation(state, { ...state, githubLists: state.githubLists.map((item) => item.id === list.id ? { ...next, items: item.items } : item) }, onStateChange, { operation: "list.update", payload: { listId: list.id, name: list.name, description: list.description, isPrivate: list.isPrivate } }); setSuccess("List 设置已保存"); } catch (reason) { setError(reason instanceof Error ? reason.message : "保存失败"); } finally { setSaving(false); }
  }
  async function removeList(list: GithubStarList) {
    if (!window.confirm(`删除 GitHub List “${list.name}”？`)) return; setSaving(true); setError("");
    try { await deleteGithubList(token, list.id); const lists = state.githubLists.filter((item) => item.id !== list.id); await runOptimisticMutation(state, { ...state, githubLists: lists }, onStateChange, { operation: "list.delete", payload: { id: list.id } }); setSelectedId(lists[0]?.id || ""); setSuccess("List 已删除"); } catch (reason) { setError(reason instanceof Error ? reason.message : "删除失败"); } finally { setSaving(false); }
  }
  async function toggleMembership(fullName: string, listId: string, active: boolean) {
    ...
    try {
      ...
      await runOptimisticMutation(state, { ...state, githubLists: optimisticLists }, onStateChange, { operation: "list.membership", payload: { repoFullName: fullName, listId, listIds: desired, active } }, { perform: async () => { await setGithubListMembership(token, fullName, desired); } });
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-5 lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="rounded-xl border border-border bg-card p-2 shadow-card"><div className="px-2 py-2 text-xs font-semibold text-muted-foreground">{state.githubLists.length} 个 Lists</div>{state.githubLists.map((list) => <button key={list.id} onClick={() => setSelectedId(list.id)} className={`mb-1 flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-sm ${selected?.id === list.id ? "bg-accent font-medium" : "text-muted-foreground hover:bg-accent/60"}`}><RiFolder3Line className="size-4" /><span className="min-w-0 flex-1 truncate">{list.name}</span><span className="text-xs">{list.items.length}</span></button>)}{!state.githubLists.length ? <div className="px-3 py-10 text-center text-sm text-muted-foreground">还没有 Lists</div> : null}</aside>
        <section className="min-w-0">
          {selected ? <div className="grid gap-4">
            <div className="rounded-xl border border-border bg-card p-4 shadow-card"><div className="grid gap-3 sm:grid-cols-2"><Field label="名称"><Input value={selected.name} onChange={(event) => onStateChange({ ...state, githubLists: state.githubLists.map((item) => item.id === selected.id ? { ...item, name: event.target.value } : item) })} /></Field><Field label="可见性"><label className="flex h-9 items-center gap-2 rounded-lg border border-input px-3 text-sm"><input type="checkbox" checked={selected.isPrivate} onChange={(event) => onStateChange({ ...state, githubLists: state.githubLists.map((item) => item.id === selected.id ? { ...item, isPrivate: event.target.checked } : item) })} />Private</label></Field></div><Field label="描述"><Textarea rows={2} value={selected.description} onChange={(event) => onStateChange({ ...state, githubLists: state.githubLists.map((item) => item.id === selected.id ? { ...item, description: event.target.value } : item) })} /></Field><div className="mt-3 flex gap-2"><Button loading={saving} onClick={() => void saveList(selected)}>保存</Button><Button variant="destructive" disabled={saving} onClick={() => void removeList(selected)}>删除</Button></div></div>
            <div className="rounded-xl border border-border bg-card p-4 shadow-card"><div className="mb-3 flex items-center gap-2"><RiSearchLine className="size-4 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索本地 Stars 并调整当前 List membership" /></div><div className="grid gap-2">{localCandidates.map((repo) => { const active = selected.items.some((item) => item.fullName === repo.full_name); return <div key={repo.full_name} className="flex items-center gap-3 rounded-lg border border-border px-3 py-2"><img src={repo.owner.avatar_url} alt="" className="size-8 rounded-md" /><div className="min-w-0 flex-1"><div className="truncate text-sm font-medium">{repo.full_name}</div><div className="truncate text-xs text-muted-foreground">{repo.description || "暂无描述"}</div></div><Button size="sm" variant={active ? "secondary" : "outline"} loading={saving} onClick={() => void toggleMembership(repo.full_name, selected.id, !active)}>{active ? "移出" : "加入"}</Button></div>; })}{!localCandidates.length ? <div className="py-10 text-center text-sm text-muted-foreground">没有匹配的 Stars</div> : null}</div></div>
