function currentUrl() {
  if (typeof window === "undefined") return new URL("https://starbox.local/");
  const location = window.location;
  const href = typeof location.href === "string" && location.href ? location.href : "";
  if (href) {
    try { return new URL(href); } catch { /* fall through to structured fields */ }
  }
...
export function readQueryParam(name: string, fallback = "") {
...
export function replaceQueryParams(values: Record<string, string | number | boolean | null | undefined>) {
  ...
  if (typeof replaceState === "function") replaceState.call(window.history, null, "", `${url.pathname}${url.search}${url.hash}`);
}
