  ...
  return (
    <Primitive.Portal>
      <Primitive.Backdrop
        ...
        className="fixed inset-0 z-[70] bg-black/32 backdrop-blur-sm transition-all duration-200 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 motion-reduce:transition-none"
      />
      <Primitive.Viewport
        data-slot="alert-dialog-viewport"
        className="fixed inset-0 z-[70] grid grid-rows-[1fr_auto_3fr] justify-items-center p-4 max-sm:grid-rows-[1fr_auto] max-sm:p-0 max-sm:pt-12"
      >
        <Primitive.Popup
          data-slot="alert-dialog-popup"
          className={cn(
            "relative row-start-2 flex max-h-full min-h-0 w-full max-w-md flex-col overflow-hidden rounded-2xl border border-border bg-popover not-dark:bg-clip-padding text-popover-foreground opacity-[calc(1-var(--nested-dialogs))] shadow-2xl outline-none transition-[scale,opacity,translate] duration-200 ease-in-out will-change-transform before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 sm:scale-[calc(1-0.1*var(--nested-dialogs))] sm:data-[ending-style]:scale-98 sm:data-[starting-style]:scale-98 max-sm:max-w-none max-sm:rounded-none max-sm:border-x-0 max-sm:border-b-0 max-sm:data-[ending-style]:translate-y-4 max-sm:data-[starting-style]:translate-y-4 dark:before:shadow-[0_-1px_--theme(--color-white/6%)] motion-reduce:transform-none motion-reduce:transition-none",
            className,
          )}
        >
          {children}
...
export function AlertDialogFooter({ children }: { children: ReactNode }) {
  return <div data-slot="alert-dialog-footer" className="flex justify-end gap-2 border-t border-border bg-secondary/30 px-5 py-4 max-sm:flex-col-reverse">{children}</div>;
}
