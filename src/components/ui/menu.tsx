...
// Keep section labels safe even when a caller does not explicitly create a Base UI Menu.Group.
// Base UI GroupLabel throws at runtime outside Group, which previously blanked the Release SPA.
export function MenuGroupLabel({ className, ...props }: HTMLAttributes<HTMLDivElement>) { return <div data-slot="menu-group-label" role="presentation" className={cn("px-2 py-1.5 text-xs font-medium text-muted-foreground", className)} {...props} />; }
