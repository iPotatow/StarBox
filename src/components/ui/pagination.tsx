...
export function PaginationPrevious({ className, children, ...props }: PaginationLinkProps) { const { t } = useI18n(); const label = children ?? t("上一页", "Previous"); return <PaginationLink aria-label={t("上一页", "Previous page")} className={cn("gap-1", className)} {...props}><RiArrowLeftSLine className="size-4" aria-hidden="true" />{label}</PaginationLink>; }
export function PaginationNext({ className, children, ...props }: PaginationLinkProps) { const { t } = useI18n(); const label = children ?? t("下一页", "Next"); return <PaginationLink aria-label={t("下一页", "Next page")} className={cn("gap-1", className)} {...props}>{label}<RiArrowRightSLine className="size-4" aria-hidden="true" /></PaginationLink>; }
export function PaginationEllipsis({ className, ...props }: HTMLAttributes<HTMLSpanElement>) { const { t } = useI18n(); return <span aria-hidden data-slot="pagination-ellipsis" className={cn("flex min-w-7 justify-center", className)} {...props}><RiMoreLine className="size-4" aria-hidden="true" /><span className="sr-only">{t("更多页面", "More pages")}</span></span>; }
