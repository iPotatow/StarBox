  ...
  return (
    <div className="app-shell min-h-screen bg-sidebar text-foreground">
      ...
      <header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-border bg-background/92 px-4 backdrop-blur md:hidden">
        ...
        <nav className="ml-auto flex min-w-0 items-center gap-1 overflow-x-auto" aria-label={t("主导航", "Main navigation")}>
          {nav.map((id) => {
            const item = navMeta[id];
            const Icon = item.icon;
            const label = t(item.zh, item.en);
            return (
              <Button key={id} variant="ghost" size="none" onClick={() => onPageChange(id)} aria-label={label} className={cn("relative grid size-9 shrink-0 place-items-center rounded-lg", page === id ? "bg-accent" : "text-muted-foreground")}>
